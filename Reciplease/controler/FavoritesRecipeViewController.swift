...
extension FavoritesRecipeViewController : UITableViewDataSource, UITableViewDelegate{
    ...
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        RecipeFavoriteTest.shared.arrayRecipeFavorite.count
    }
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let recipe = RecipeFavoriteTest.shared.arrayRecipeFavorite[indexPath.row]
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        recipeDetail = RecipeFavoriteTest.shared.arrayRecipeFavorite[indexPath.row]
        currentImage = UIImage(data: (RecipeFavoriteTest.shared.arrayRecipeFavorite[indexPath.row].recipe?.imageData)!)
        self.performSegue(withIdentifier: "SegueToDetailRecipeViewControllerViaFav", sender: nil)
    }
    
    func tableView(_ tableView: UITableView, commit editingStyle: UITableViewCell.EditingStyle, forRowAt indexPath: IndexPath) {
        if editingStyle == .delete {
            RecipeFavoriteTest.shared.removeRecipe(at: indexPath.row)
